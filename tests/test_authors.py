...
def test_authors_no_records(client):
...
def test_get_single_author(client, sample_data):
    response = client.get('/api/v1/authors/3')

    response_data = response.get_json()
    assert response.status_code == 200
    assert response.headers['Content-Type'] == 'application/json'
    assert response_data['success'] is True
    assert response_data['data']['first_name'] == 'Roald'
    assert response_data['data']['last_name'] == 'Dahl'
    assert response_data['data']['birth_date'] == '13-09-1916'
    assert len(response_data['data']['books']) == 1
    assert response_data['data']['id'] == 3


def test_get_single_author_not_found(client):
    response = client.get('/api/v1/authors/54')
...
def test_create_author(client, author, token):
...
@pytest.mark.parametrize(
    'data, missing_field',
    [
        ({'first_name': 'Jan', 'last_name': 'Kowalski'}, 'birth_date'),
        ({'first_name': 'Jan', 'birth_date': '11-04-1900'}, 'last_name'),
        ({'birth_date': '11-04-1900', 'last_name': 'Kowalski'}, 'first_name'),
    ]
)
...
def test_update_author(client, token, sample_data, author):
    response = client.put('/api/v1/authors/5',
                          json=author,
                          headers={
                              'Authorization': f'Bearer {token}'
                          })
...
def test_update_author_invalid_data(client, token, sample_data, data, missing_field):
    response = client.put('/api/v1/authors/5',
                          json=data,
                          headers={
                              'Authorization': f'Bearer {token}'
...
def test_update_author_invalid_content_type(client, token, sample_data, author):
    response = client.put('/api/v1/authors/2',
                          data=author,
                          headers={
                              'Authorization': f'Bearer {token}'
                          })
...
def test_update_author_missing_token(client, sample_data, author):
    response = client.put('/api/v1/authors/7', json=author)
...
def test_update_author_not_found(client, token, author, sample_data):
    response = client.put('/api/v1/authors/53',
                          json=author,
                          headers={
                              'Authorization': f'Bearer {token}'
                          })
...
def test_delete_author(client, sample_data, token):
    response = client.delete('/api/v1/authors/2',
                             headers={
                                 'Authorization': f'Bearer {token}'
                             })
...
def test_delete_author_not_found(client, sample_data, token):
    response = client.delete('/api/v1/authors/78',
                             headers={
                                 'Authorization': f'Bearer {token}'
                             })
    response_data = response.get_json()
